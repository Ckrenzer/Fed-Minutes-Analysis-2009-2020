...
There are two formats for the HTML file--those the Fed released before 2012 and those released after. Therefore, two functions are used to read in the different formats. I used for loops to create the dataset because the amount of overhead required to use loop functions in this context is unjustified.
```{r, echo = TRUE}
# Reading in all the files to get their names into a character vector
report_names <- str_remove_all(list.files("Fed Minutes Releases/Sample/"), "\\.pdf")


# The functions to read in and clean the two formats:
# For the Pre-2012 releases
old_format <- function(id){
  name <- str_c("https://www.federalreserve.gov/monetarypolicy/",
                id,
                ".htm")
  ...
  tibble(doc_id = id, text = read_html(name) %>% 
           ...
           str_c(., collapse = " ") %>%
           str_to_lower() %>% 
           str_replace_all("'s|s'|\"|,|:|;|-{2,}|\\s+\\.\\s+", " ") %>% 
           str_squish() %>% 
           str_remove_all("return to text[\\d]*|return to top") %>% 
           str_trim()
  ) %>% 
    return()
...
# For releases 2012 and on
# We can always assume that the first 37 entries are bogus
...
# Creating a tibble to store all the documents
report_names <- str_remove_all(list.files(dirs), "\\.pdf")


documents <- tibble(doc_id = NULL, text = NULL)
...
## Preprocessing
This analysis will be performed using tidy data principles.
