...
What should instead be done, as implied above, is to break the documents into smaller pieces and calculate correlation coefficients for each of these subsections.
...
documents %>% 
  mutate(doc_id = lubridate::as_date(stringr::str_sub(doc_id, start = 12))) %>% 
  tidytext::unnest_paragraphs(paragraph, text, paragraph_break = ".")
...
# In particular, here we'll focus on the phi coefficient, a common measure for binary correlation. The phi coeffient
