...
# The trigrams
trigrams_unnested_documents <- documents %>% 
  tidytext::unnest_tokens(word, text, token = "ngrams", n = 3)

trigrams_unnested_documents_filtered <- trigrams_unnested_documents %>% 
...
# Good for the bar charts
trigrams_unnested_documents_filtered %>% 
  count(word, sort = TRUE) %>% 
  slice(1:10) %>% 
  ggplot(aes(x = reorder(word, n), y = n, fill = word)) +
  geom_col(show.legend = F)+
...
These terms might benefit from exploring combinations of four words to find words that precede these phrases, such as "_strong_ labor market conditions" or "_long_ term inflation expectations."
...
this is the cool part---you break the terms into their n-grams, then plot them over time!
