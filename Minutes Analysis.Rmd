...
knitr::kable(tibble(doc_id = documents$doc_id[1], text = str_sub(documents$text[1], end = 510)))

```


The table above shows an excerpt from the Minutes release for the meeting dated January 28th, 2009. You might notice that some words are concatenated together, such as "mattersby" (which is actually written "matters by" in the document itself). No algorithm is perfect, but you can see that these types of errors are infrequent and therefore unlikely to skew the results of the analysis.