...
knitr::kable(tibble("Fed Minutes Text" = stringr::str_sub(documents$text[1], end = 510)))

```


The table above shows an excerpt from the Minutes release for the meeting dated January 28th, 2009. You might notice that some words have parsing irregularities, such as "mattersby" (which is actually written "matters by" in the document) on the second line. No algorithm is perfect, but one can see these errors are infrequent and therefore unlikely to skew the results of the analysis.
