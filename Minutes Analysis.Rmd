...
cpi <- readr::read_csv("CPI Nov 2017 = 100.csv",
                       col_names = T)



# I'm not familiar with any functions that perform a percent change,
# but this for loop will take care of that--it's short so
# I don't feel too guilty.
for(i in 13:nrow(cpi)){
  cpi[i, 2] <- unlist((cpi[i, 2] - cpi[i - 12, 2]) / cpi[i - 12, 2] * 100)
}
cpi <- cpi[13:nrow(cpi), ]
...
ggplot() +
  geom_line(data = fed_sentiment,
            mapping = aes(x = doc_id, y = sentiment, color = "red")) +
  geom_line()
