...
There are two formats for the HTML file--those the Fed released before 2012 and those released after. Therefore, two functions are used to read in the different formats. The doucments are read in chronological order, but they are arranged after being added to the tibble if something were to happen to the order of the files in the "Fed Minutes Releases" folder from which these names were pulled.
```{r, echo = TRUE}
...
After removing 'stop words' (Ex. "the," "of," "a," "to," etc.), those words that are extremely common and provide very little meaning, which words does Brian use most frequently?
```{r, echo = FALSE}
...
These are words one expects to find in official banking documents. The terms "committee," "inflation," and "market" dominate the content of the phrasing. Still, though, many of these words are simply generic banking lingo we cannot derive much meaning from.

To extract words that have more significance in each document, we can instead use the tf-idf statistic.
