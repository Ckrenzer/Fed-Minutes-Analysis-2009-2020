...
Look at that dip at the beginning of the 2020 recession! They don't call economics the dismal science for nothing... Why don't we use an index to compare the sentiment from the FOMC Minutes with the S&P 500?



```{r echo = TRUE, results = "show"}

# we have fed_sentiment for one geom
# and we'll have the cpi for another geom


# For the fed...
# Make the 'sentiment' column an index



# full join the datasets and then you can plot their values on the same geom using a legend
...
# I will be making the index column an index--odd, yes, but
# it will allow this graph to scale with the fed data





fed_sentiment$sentiment <- (fed_sentiment[[2]] / fed_sentiment[[2]][71]) * 100




ggplot() +
  geom_line(data = fed_sentiment,
            aes(x = doc_id, y = sentiment, color = "fomc"),
            size = 1.02, show.legend = T) +
  geom_line(data = bonds,
            aes(x = date, y = ind, color = "bond"),
                size = 1.02, show.legend = T) +
  xlab("Date") +
  ylab("Index (Base: Nov 1, 2017)") +
  ggtitle("FOMC Minutes Sentiment vs. 10 Year Treasury Maturity Rate") +
  theme_bw()
