...
documents %>% 
  ...
  tidyr::separate(col = "word", into = c("word1", "word2", "word3")) %>% 
  filter(!word1 %in% tidytext::stop_words$word,
         !word1 %in% c("federal", "reserve", "committee", "intermeeting"),
         !word2 %in% tidytext::stop_words$word,
         !word2 %in% c("federal", "reserve", "committee", "intermeeting"),
...
Now that we have terms stored as bigrams (and trigrams!), we can perform sentiment analysis on the Fed Minutes releases. Various lexicons have been built up over the years to determine the mood that a document gives off. In this case, the Louhgran and McDonald dictionary of financial sentiment terms (Loughran and McDonald 2011). Other popular lexicons, such as the "AFINN," "NRC," or "BING" are not suited for documents coming from a bank. These three lexicons assign values to terms such as "share" or "risk" when they may actually be neutral in a financial context.
