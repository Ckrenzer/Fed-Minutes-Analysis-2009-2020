...
```
We don't have to compare the words to other words, of course. Why don't we compare the Fed's use of "inflation" with the inflation rate growth, using an index? Only two Fed Minutes releases started on the first day of the month in the sample period, so the base will be set to 100 on November 1st, 2017. This is because the base to the CPI can only be set on a per-month basis.

```{r echo = TRUE, results "show"}

cpi_actual <- readr::read_csv("(seas) Core CPI Index Nov 2017 = 100.csv")

cpi_mentions <- unigram_unnested_documents %>% 
  count(doc_id, word) %>% 
  ungroup() %>% 
  ...
  ggplot() +
  geom_line(aes(x = doc_id, y = n), color = "red") +
  geom_line(data = ,
            aes(x = date, y = cpi), color = "blue")
